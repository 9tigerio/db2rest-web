...
It is imperative to restrict access to the database from external sources. The database dashboard already shows warning.
...
Now the database instance is safe and only allowed connection request from trusted sources.
